...
def main():
    ...
    polo = Poloniex(Key=myAPIKey, Secret=mySecret, timeout=10, coach=True, loglevel=logging.WARNING, extend=False)
    print(polo.getSummary())
...
class Poloniex(object):
    ...
    def returnSummary(self):
        summary = self.returnMarginAccountSummary()
        tmpStr = ""
        for k, v in summary.items():
            tmpStr += k + "   " + v + "\n"
        return tmpStr

    def getOpenMarginPosition(self):
        position = self.getMarginPosition()
        tmpStr = ""
        for k, v in position.items():
            if v["amount"] != "0.00000000":
                tmpStr += k + " " + str(v) + "\n"
        if tmpStr == "":
            return "Nothing"
        else:
    ...
    def returnEstimatedValueOfHoldings(self):
        ...
        summary = self.returnMarginAccountSummary()
        estimatedValueOfHoldingsAsBTC = float(summary["netValue"])
        lastValueUSDT_BTC = self.returnTicker()["USDT_BTC"]["last"]
    ...
    def returnOpenMarginOrders(self):
        onOrders = self.returnOpenOrders(pair="all")
        tmpStr = ""
        for k, v in onOrders.items():
            if v!=[]:
                if v[0]["margin"]==1:
                    tmpStr += k + " " + str(v[0]) + "\n"
        if tmpStr == "":
    ...
    def getSummary(self):
        ...
        positions = self.getOpenMarginPosition()
        onMarginOrders = self.returnOpenMarginOrders()

        summaryStr = ""
        summaryStr += "-"*35 + "\n"
        summaryStr += "Time: " + str(datetime.datetime.today())[0:19] + "\n"
        summaryStr += "\n"
